...
const Row = ({ test } : rowProps) => {
  return (
    <div className={'border-gray-500 border-b-2 w-full max-h-12 min-h-12 items-center flex justify-between'}>
      <div className={'text-md'}>{test.test}</div>
      <div className={'ml-auto flex w-[30%] justify-between'}>
        <div className={'text-xl w-[33%] self-center flex justify-center'}>{test.concept}</div>
        <div className={'text-xl pr-8 w-[33%] flex justify-center'}>{test.output}</div>
        <Buttons/>
      </div>
    </div>
  )
